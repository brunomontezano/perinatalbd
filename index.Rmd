...
-   `traumatismo`: O bebê apresentou traumatismo no parto?
...
-   `baixa_escol_paterna`: Baixa escolaridade paterna (\<= 3 anos de
    estudo)
...
pequeno fluxo para realizar a imputação das variáveis por meio de média
...
Primeiramente foi elaborado um modelo multivariado com todas as
variáveis, e logo na sequência, um modelo contendo somente as variáveis
com valor *p* menor que 0,2 na análise bivariada.
...
mod_fit |> 
    ...
    dplyr::filter(
        variavel != "(Intercept)" & or < 10 & or > 0.5
        ) |> 
    ggplot2::ggplot() +
...
#### Análise multivariada somente com variáveis com p \< 0,2 na análise bruta

Neste modelo, foram incluídas somente as variáveis independentes que
apresentaram um valor de p menor que 0,2 na análise bivariada.
