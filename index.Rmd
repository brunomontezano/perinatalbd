...
```{r}
dados_com_na <- haven::read_sav("data/banco_QI_15_07.sav",
                                user_na = TRUE)
dados_novos <- dados_com_na |> 
    dplyr::mutate(
        outcome = dplyr::if_else(
            MDD_BD1_2_notmood %in% 2:3 |
            TB_final_original %in% c(1, 999) |
            TB1_18anos == 1 | TBII.p1_18anos == 1,
            "Sim",
            "Não",
            missing = "Não"
        )
